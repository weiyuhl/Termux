...
class RunCommandService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        executionCommand.pluginAPIHelp = getString(
        ...
        if (RUN_COMMAND_SERVICE.ACTION_RUN_COMMAND != intent.action) {
            errmsg = getString(R.string.error_run_command_service_invalid_intent_action, intent.action)
         ...
         */
        val replaceCommaAlternativeCharsInArguments = intent.getBooleanExtra(
            RUN_COMMAND_SERVICE.EXTRA_REPLACE_COMMA_ALTERNATIVE_CHARS_IN_ARGUMENTS,
            false
        )
        if (replaceCommaAlternativeCharsInArguments) {
            var commaAlternativeCharsInArguments = IntentUtils.getStringExtraIfSet(
                intent,
                RUN_COMMAND_SERVICE.EXTRA_COMMA_ALTERNATIVE_CHARS_IN_ARGUMENTS,
                null
            )
            ...
            DataUtils.replaceSubStringsInStringArrayItems(
                executionCommand.arguments,
                commaAlternativeCharsInArguments,
        ...
        executionCommand.runner = IntentUtils.getStringExtraIfSet(
            intent, RUN_COMMAND_SERVICE.EXTRA_RUNNER,
            if (intent.getBooleanExtra(RUN_COMMAND_SERVICE.EXTRA_BACKGROUND, false))
        ...
        executionCommand.backgroundCustomLogLevel = IntentUtils.getIntegerExtraIfSet(
            intent,
            RUN_COMMAND_SERVICE.EXTRA_BACKGROUND_CUSTOM_LOG_LEVEL,
            null
        )
        ...
        executionCommand.shellCreateMode = IntentUtils.getStringExtraIfSet(
            intent,
            RUN_COMMAND_SERVICE.EXTRA_SHELL_CREATE_MODE,
            null
        )
        ...
        executionCommand.commandDescription = IntentUtils.getStringExtraIfSet(
            intent,
        ...
        executionCommand.resultConfig.resultDirectoryPath = IntentUtils.getStringExtraIfSet(
            intent,
            RUN_COMMAND_SERVICE.EXTRA_RESULT_DIRECTORY,
            null
        )

        if (executionCommand.resultConfig.resultDirectoryPath != null) {
            executionCommand.resultConfig.resultSingleFile = intent.getBooleanExtra(
                RUN_COMMAND_SERVICE.EXTRA_RESULT_SINGLE_FILE,
                false
            )
            executionCommand.resultConfig.resultFileBasename = IntentUtils.getStringExtraIfSet(
                intent,
                RUN_COMMAND_SERVICE.EXTRA_RESULT_FILE_BASENAME,
                null
            )
            executionCommand.resultConfig.resultFileOutputFormat = IntentUtils.getStringExtraIfSet(
                intent,
                RUN_COMMAND_SERVICE.EXTRA_RESULT_FILE_OUTPUT_FORMAT,
                null
            )
            executionCommand.resultConfig.resultFileErrorFormat = IntentUtils.getStringExtraIfSet(
                intent,
                RUN_COMMAND_SERVICE.EXTRA_RESULT_FILE_ERROR_FORMAT,
                null
            )
            executionCommand.resultConfig.resultFilesSuffix = IntentUtils.getStringExtraIfSet(
                intent,
                RUN_COMMAND_SERVICE.EXTRA_RESULT_FILES_SUFFIX,
                null
            )
        }
        ...
        if (executionCommand.executable.isNullOrEmpty()) {
            errmsg = getString(
                R.string.error_run_command_service_mandatory_extra_missing,
        ...
        if (!executionCommand.workingDirectory.isNullOrEmpty()) {
            ...
            executionCommand.workingDirectory = TermuxFileUtils.getCanonicalPath(
            ...
            error = TermuxFileUtils.validateDirectoryFileExistenceAndPermissions(
        ...
        if (FileUtils.getFileType(executableExtra, false) == FileType.SYMLINK) {
            Logger.logVerbose(
                LOG_TAG,
                "The executableExtra path \"$executableExtra\" is a symlink so using it instead of the canonical path \"${executionCommand.executable}\""
            )
            executionCommand.executable = executableExtra
        }

        executionCommand.executableUri = Uri.Builder()
            .scheme(TERMUX_SERVICE.URI_SCHEME_SERVICE_EXECUTE)
            .path(executionCommand.executable)
            .build()
    ...
    private fun setupNotificationChannel() {
        ...
        NotificationUtils.setupNotificationChannel(
            this, TermuxConstants.TERMUX_RUN_COMMAND_NOTIFICATION_CHANNEL_ID,

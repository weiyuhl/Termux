...
class TermuxService : Service(), AppShell.AppShellClient, TermuxSession.TermuxSessionClient {
    ...
    private fun actionServiceExecute(intent: Intent?) {
        ...
        if (Runner.runnerOf(executionCommand.runner) == null) {
            val errmsg = getString(
                R.string.error_termux_service_invalid_execution_command_runner,
    ...
    private fun executeTermuxTaskCommand(executionCommand: ExecutionCommand?) {
        ...
        if (ShellCreateMode.NO_SHELL_WITH_NAME == shellCreateMode) {
            ...
            if (newTermuxTask != null) {
                Logger.logVerbose(
                    LOG_TAG,
                    "Existing TermuxTask with \"" + executionCommand.shellName +
                            "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\""
                )
            } else {
                Logger.logVerbose(
                    LOG_TAG,
                    "No existing TermuxTask with \"" + executionCommand.shellName +
                            "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\""
                )
            }
        }
    ...
    fun createTermuxTask(executionCommand: ExecutionCommand?): AppShell? {
        ...
        Logger.logDebug(
            LOG_TAG,
            "Creating \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TermuxTask"
        )
    ...
    override fun onAppShellExited(appShell: AppShell) {
        mHandler.post {
            val executionCommand = appShell.executionCommand

            Logger.logVerbose(
                LOG_TAG,
                "The onTermuxTaskExited() callback called for \"" +
            ...
            mShellManager.mTermuxTasks.remove(appShell)
    ...
    private fun executeTermuxSessionCommand(executionCommand: ExecutionCommand?) {
        ...
        Logger.logDebug(
            LOG_TAG,
            "Executing foreground \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TermuxSession command"
        )
        ...
        if (ShellCreateMode.NO_SHELL_WITH_NAME == shellCreateMode) {
            ...
            if (newTermuxSession != null) {
                Logger.logVerbose(
                    LOG_TAG,
                    "Existing TermuxSession with \"" + executionCommand.shellName +
                            "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\""
                )
            } else {
                Logger.logVerbose(
                    LOG_TAG,
                    "No existing TermuxSession with \"" + executionCommand.shellName +
                            "\" shell name found for shell create mode \"" + shellCreateMode.getMode() + "\""
                )
            }
        }
    ...
    fun createTermuxSession(executionCommand: ExecutionCommand?): TermuxSession? {
        ...
        Logger.logDebug(
            LOG_TAG,
            "Creating \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TermuxSession"
        )

        if (!Runner.TERMINAL_SESSION.equalsRunner(executionCommand.runner)) {
            Logger.logDebug(
                LOG_TAG,
                "Ignoring wrong runner \"" + executionCommand.runner + "\" command passed to createTermuxSession()"
            )
            return null
        }
    ...
    override fun onTermuxSessionExited(termuxSession: TermuxSession) {
        ...
        Logger.logVerbose(
            LOG_TAG,
            "The onTermuxSessionExited() callback called for \"" +
    ...
    private fun processShellCreateMode(executionCommand: ExecutionCommand): ShellCreateMode? {
        return when {
            ShellCreateMode.ALWAYS.equalsMode(executionCommand.shellCreateMode) -> {
                ShellCreateMode.ALWAYS // Default
            }
            ShellCreateMode.NO_SHELL_WITH_NAME.equalsMode(executionCommand.shellCreateMode) -> {
                if (DataUtils.isNullOrEmpty(executionCommand.shellName)) {
                    TermuxPluginUtils.setAndProcessPluginExecutionCommandError(
                        ...
                        getString(
                            R.string.error_termux_service_execution_command_shell_name_unset,
                ...
                TermuxPluginUtils.setAndProcessPluginExecutionCommandError(
                    this, LOG_TAG, executionCommand, false,
                    getString(
                        R.string.error_termux_service_unsupported_execution_command_shell_create_mode,
                        executionCommand.shellCreateMode
                    )
                )
                null
    ...
    fun setTermuxTerminalSessionClient(termuxTerminalSessionActivityClient: TermuxTerminalSessionActivityClient?) {
        ...
        for (i in mShellManager.mTermuxSessions.indices) {
            mShellManager.mTermuxSessions[i].terminalSession.updateTerminalSessionClient(
    ...
    val lastTermuxSession: TermuxSession?
        get() = if (mShellManager.mTermuxSessions.isEmpty()) {
            null
        } else {
            mShellManager.mTermuxSessions[mShellManager.mTermuxSessions.size - 1]
        }
    ...
    fun getTerminalSessionForHandle(sessionHandle: String?): TerminalSession? {
        for (i in 0 until mShellManager.mTermuxSessions.size) {
            val terminalSession = mShellManager.mTermuxSessions[i].terminalSession
    ...
    fun getTermuxTaskForShellName(name: String?): AppShell? {
        ...
        for (i in 0 until mShellManager.mTermuxTasks.size) {
            val appShell = mShellManager.mTermuxTasks[i]
    ...
    fun getTermuxSessionForShellName(name: String?): TermuxSession? {
        if (DataUtils.isNullOrEmpty(name)) return null
        for (i in 0 until mShellManager.mTermuxSessions.size) {
            val termuxSession = mShellManager.mTermuxSessions[i]
